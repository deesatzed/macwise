...
| D-041 | 2026-07-20 | Permit automatic checkup-only public identification for an unresolved application, using a bounded one-app request and a visible `--offline` escape hatch. The feature sends no inventory upload and has no account, telemetry, or background update. Public facts are not authoritative for cleanup. This supersedes only the live-lookup prohibition in `GOAL_SIMPLE_UX.md`; D-022 keeps overlap analysis deterministic and exact-match, while `macwise-eval` remains network-free. | A common application can lack a local purpose even when its local inventory is correct. Narrow, cited identification improves first-run comprehension without exposing a software list or allowing web information to influence usage, overlap, planning, apply, or undo safety decisions. | Accepted |

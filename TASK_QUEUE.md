...
| MW-604 | 8 | Build the independent evaluation lab in `GOAL_EVAL.md` | in progress | Isolated evaluator boundary and standalone CLI pass; remaining evidence capsule, privacy, frozen policy, claim/metric, mutation, action, version, hosted-macOS, and private real-Mac gates remain |
| MW-605 | 8/Later | Run the blinded external usefulness and safety pilot | deferred | Begin only after MW-604 passes and the user separately authorizes recruitment, compensation budget, consent process, and aggregate data return |
